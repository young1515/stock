...
class Bong:
    ...
    def __init__(self, data):
        self.open = data[0].close
        self.close = data[-1].close
        self.high = 0
        self.low = 1e100
        for i in data:
            if i.close >= self.high:
                self.high = i.close
        for i in data:
            if i.close <= self.low:
                self.low = i.close
...
class Maindict():
    ...
    def make_dic_set(self):
        ...
        for i in range(len(row_company)):
            ...
            self.calcul_dict[row_company[i]['code']].update(
                {'name': row_company[i]['company']})
    ...
    def make_mins(self, unit_time):
        ...
        for idx in range(len(row_tick_data)):
            ...
            if code not in bunbong:
                bunbong[code] = BunBong(
                    time_data, time_data, 0, code, open_data)
            ...
            if timedelta(minutes=min_data_code) <= (time_data-started_time[code]):
                ...
                bunbong[code] = BunBong(
                    time_data, time_data, min_data_code, code, open_data)
